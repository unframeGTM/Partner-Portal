...
// code — see pages/api/auth.js. OFF unless explicitly enabled: set
// ENABLE_TEST_LOGINS=true and TEST_PARTNER_LOGINS='{"me@gmail.com":"deloitte.com"}'.
// There is no built-in bypass address, so production is code-verified by default.
export function getTestLoginDomain(email) {
  if (process.env.ENABLE_TEST_LOGINS !== 'true') return null;
  const map = {};
  try {
...
// Internal Unframe users who get the admin view (all partner accounts). Override
// with ADMIN_EMAILS='a@x.com,b@x.com'.
export function getAdminEmails() {
  const defaults = ['lex.levitte@unframe.ai', 'connie.nguyen@unframe.ai', 'ryan.roberts@unframe.ai'];
  const set = new Set(defaults.map(e => e.toLowerCase()));
...
// Every recognized partner organization, with partner-opportunity rollups.
export async function getAllPartnerAccounts() {
  const conn = await getSFConnection();
  const result = await conn.query(`
    SELECT Id, Name, Partner_Type__c, Website_Domain__c,
           Open_Opps_as_Partner__c, Won_Opps_as_Partner__c
    FROM Account
